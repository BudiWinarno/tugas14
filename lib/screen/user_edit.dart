...
class FormEditUser extends StatefulWidget {
  ...
  FormEditUser({@required this.user, @required this.id, Key key})
      : super(key: key);
  ...
  void initState() {
    ...
    if (widget.user != null && widget.id != null) {
      ...
      _fullnameController.value = TextEditingValue(
          ...
          selection:
              TextSelection.collapsed(offset: widget.user.fullName.length));
          ...
          onPressed: () {
            ...
            if (form.validate()) {
              ...
              if (_grade == null) {
                ...
                final onError =
                    (Object error) => _showSnackBar("Tidak bisa ubah data");

                apiService
                    .updateUser(id: widget.user.id, data: _user)
                    .then(onSuccess)
                    .catchError(onError);
              }
            } else {
